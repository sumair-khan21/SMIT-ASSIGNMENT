...
const EditCategoryModal = ({ isOpen, onClose, onSuccess, category }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await adminCategoryOperations.update(category.id, {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white/80 text-sm">ID: {category.id}</p>
              </div>
            </div>
            <button
              onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="flex space-x-4 pt-4 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-pink-600 text-white font-semibold rounded-xl hover:shadow-xl transform hover:scale-105 transition-all duration-200 disabled:opacity-50"
            >
