let products =  [
    ...
    <div class="card">
        ...
        <span>Brand: ${products[i].brand}</span>
        <span>Price: ${products[i].price}</span>  
        <span>Rating: ${products[i].rating}</span>
        <img src="${products[i].images}" class="img">
    </div>
        `
    }
...
searchBtn.addEventListener('click', function(){
    ...
    for(let i = 0; i < cards.length; i++){
        let title = cards[i].querySelector('h2').textContent.toLowerCase()
