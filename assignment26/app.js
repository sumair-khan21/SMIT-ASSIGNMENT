let recipes =  [
    ...
    for(let i = 0; i < recipes.length; i++){
        console.log(recipes[i].category);
         
        show.innerHTML += `
        <div class="card">
        <img src="${recipes[i].image}" alt="" class="card-img">
        <div class="card-content">
        <p><b>Name: </b> ${recipes[i].name}</p>
            <p><b>Cuisine: </b> ${recipes[i].cuisine}</p>
            <p><b>Rating: </b> ${recipes[i].rating}</p>
        </div>
...
// let recipes = [
//     { name: "Dish 1", cuisine: "Asian", difficulty: "Easy", tags: ["Vegetarian", "Stir-fry", "Asian"] },
//     { name: "Dish 2", cuisine: "Italian", difficulty: "Medium", tags: ["Pasta", "Cheese"] },
//     { name: "Dish 3", cuisine: "Mexican", difficulty: "Hard", tags: ["Spicy", "Taco"] },
//     { name: "Dish 4", cuisine: "Asian", difficulty: "Hard", tags: ["Noodles", "Spicy"] },
//     { name: "Dish 5", cuisine: "French", difficulty: "Easy", tags: ["Baking", "Sweet"] }
// ];

// ✅ Filters ko dynamically populate karna
function populateFilters() {
    let cuisines = new Set(), difficulties = new Set(), tags = new Set();
    recipes.forEach(r => {
    ...
    // ✅ Dropdown ko reset karna aur phir unique values add karna
    cuisineFilter.innerHTML = `<option value="">All</option>`;
    cuisines.forEach(c => cuisineFilter.innerHTML += `<option value="${c}">${c}</option>`);

    difficultyFilter.innerHTML = `<option value="">All</option>`;
    difficulties.forEach(d => difficultyFilter.innerHTML += `<option value="${d}">${d}</option>`);

    tagFilter.innerHTML = `<option value="">All</option>`;
    tags.forEach(t => tagFilter.innerHTML += `<option value="${t}">${t}</option>`);
}

// ✅ Filtered Recipes ko show karna
function filterRecipes() {
    ...
    let filtered = recipes.filter(r => 
        ...
        (selectedTag === "" || r.tags.includes(selectedTag))
    );

    recipeContainer.innerHTML = filtered.length 
        ? filtered.map(r => `<p>${r.name} - ${r.cuisine} - ${r.difficulty} - ${r.tags.join(", ")}</p>`).join("")
        : "<p>No recipes found</p>";
}

// ✅ Pehle filters ko populate karna, phir event listeners lagana
populateFilters();
[cuisineFilter, difficultyFilter, tagFilter].forEach(f => f.addEventListener("change", filterRecipes));
filterRecipes();
